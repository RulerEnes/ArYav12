...
exports.run = async (client, message, args) => {
    const error = (str) => message.channel.send(new Discord.MessageEmbed().setColor('#00f4fd').setTitle('Hata').setDescription(str));
    ...
    const unban = async (user) => {
        ...
        try {
            ...
            user.send(new Discord.MessageEmbed().setColor('#00f4fd').setDescription(`> (#${ban.id}) **${message.guild.name}** adlı sunucudan banınız açıldı.`));
            message.channel.send(new Discord.MessageEmbed().setColor('#00f4fd').setDescription(`> (#${ban.id}) **${user.tag}** kişisine atılan açılmayan ban açıldı.`));
    ...
    const list = async () => {
        ...
        const embed = new Discord.MessageEmbed().setColor('#00f4fd').setDescription(`Toplam **${bans.length}** kişi banlı. Açılmayan banlılar listesini aşağıdaki dosyadan görebilirsiniz.`);
        const attachment = new Discord.MessageAttachment(Buffer.from(banneds + '\n\n\nArYa botu kullandığınız  için teşekkür ederiz❤️', 'utf-8'), 'açılmayan-ban-listesi.txt');
...
exports.help = {
    name: 'açılmayan-ban',
    description: 'açılmayan ban sistemi ile birisi banladığı zaman ban daha açılmaz.'
};