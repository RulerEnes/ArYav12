...
var prefix = ayarlar.prefix;             
  ...
  if (message.mentions.users.size < 1) return message.channel.send(`Komutu yanlış kullandınız örnek kullanım ==>a.kick <@kullanıcı> <sebep>; `);
  if (user.id === message.author.id) return message.channel.send('Kendini atamazsın sunucudan uzaklaşmak istiyorsan bir yetkiliden seni atmasını isteyebilirsin.');
